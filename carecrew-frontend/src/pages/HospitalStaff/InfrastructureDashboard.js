...
const InfrastructureDashboard = ({ hasCapacity, cap, diseaseHistory, navigate }) => {
  return (
    <div className="flex flex-col gap-6">
      {!hasCapacity ? (
        ...
        <>
          {/* Real-Time Resource Burden Analysis */}
          {(() => {
            ...
            return (
              <div className={`w-full rounded-2xl border p-6 shadow-sm mb-6 ${styles.bg} ${styles.border}`}>
                ...
                <p className={`text-base leading-relaxed font-medium opacity-90 ${styles.text} max-w-4xl`}>
                  {bDesc}
                </p>
              </div>
            );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <Card title="General Beds">
              <div className="flex flex-col mt-2">
                ...
                <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                  <div className={`h-2 rounded-full ${cap.availableBeds === 0 ? 'bg-red-500' : 'bg-green-500'}`} style={{ width: `${cap.totalBeds > 0 ? (cap.availableBeds / cap.totalBeds) * 100 : 0}%` }} />
                </div>
                <p className="text-xs text-gray-500 text-right">{cap.totalBeds > 0 ? Math.round((cap.availableBeds / cap.totalBeds) * 100) : 0}% capacity remaining</p>
              </div>
            </Card>
            ...
            <Card title="ICU Beds">
              <div className="flex flex-col mt-2">
                ...
                <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                  <div className="h-2 rounded-full bg-blue-500" style={{ width: `${cap.icuTotal > 0 ? (cap.icuAvailable / cap.icuTotal) * 100 : 0}%` }} />
                </div>
                <p className="text-xs text-gray-500 text-right">{cap.icuTotal > 0 ? Math.round((cap.icuAvailable / cap.icuTotal) * 100) : 0}% capacity remaining</p>
              </div>
            </Card>
            ...
            <Card title="Overall Bed Occupancy">
              <div className="flex flex-col mt-2">
                ...
                <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                  <div className="h-2 rounded-full bg-orange-500" style={{ width: `${cap.totalBeds > 0 ? ((cap.totalBeds - cap.availableBeds) / cap.totalBeds) * 100 : 0}%` }} />
                </div>
            ...
            <Card title="Oxygen Supply">
              <div className="flex flex-col mt-2 h-[88px]">
                ...
                <div className="flex items-center justify-between mt-auto">
                  <p className="text-xs text-gray-400">Total capacity: <span className="font-semibold text-gray-600">{cap.oxygenTotal || 0}</span></p>
                  <span className={`px-2.5 py-1 text-[10px] uppercase tracking-wider font-bold rounded-full border ${cap.oxygenStatus === 'Critical' ? 'bg-red-50 text-red-600 border-red-200' : cap.oxygenStatus === 'Low' ? 'bg-yellow-50 text-yellow-600 border-yellow-200' : 'bg-green-50 text-green-600 border-green-200'}`}>{cap.oxygenStatus}</span>
                </div>
              </div>
            </Card>
            ...
            <Card title="Medicine Stock">
              <div className="flex flex-col mt-2 h-[88px]">
                <div className="flex items-center gap-3 mb-4">
                  <span className={`text-3xl font-bold ${cap.medicineStockPercentage < 20 ? 'text-red-600' : 'text-gray-800'}`}>{cap.medicineStockPercentage}%</span>
                  <span className="text-sm text-gray-500">stock remaining</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-1.5 mb-3">
                  <div className={`h-1.5 rounded-full ${cap.medicineStockPercentage < 20 ? 'bg-red-500' : cap.medicineStockPercentage < 50 ? 'bg-yellow-400' : 'bg-green-500'}`} style={{ width: `${Math.min(100, cap.medicineStockPercentage)}%` }} />
                </div>
                <div className="flex items-center justify-end mt-auto">
                  <span className={`px-2.5 py-1 text-[10px] uppercase tracking-wider font-bold rounded-full border ${cap.medicineStockPercentage < 20 ? 'bg-red-50 text-red-600 border-red-200' : cap.medicineStockPercentage < 50 ? 'bg-yellow-50 text-yellow-600 border-yellow-200' : 'bg-green-50 text-green-600 border-green-200'}`}>{cap.medicineStatus}</span>
                </div>
              </div>
            </Card>
          </div>
